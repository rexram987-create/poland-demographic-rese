(function () {
  ...
  function getLanguageTarget(en){const current=location.pathname.split("/").pop()||"index.html";if(current==="index.html")return"index-en.html";if(current==="index-en.html")return"index.html";if(current==="jewish-poland.html")return"jewish-poland-en.html";if(current==="jewish-poland-en.html")return"jewish-poland.html";return en?current.replace("-en.html",".html"):current.replace(".html","-en.html");}
  ...
  function buildNav(){if(document.querySelector(".geh-nav-shell"))return;const en=isEnglishPage();const homePath=en?"index-en.html":"index.html";const jewishPath=en?"jewish-poland-en.html":"jewish-poland.html";const languagePath=getLanguageTarget(en);const nav=document.createElement("div");nav.className="geh-nav-shell";nav.innerHTML=`
    <div class="geh-nav-inner">
      <a class="geh-brand" href="${withPrefix(homePath)}" aria-label="${en?"Home":"דף הבית"}"><span class="geh-brand-mark"><i class="fa-solid fa-house"></i></span><span>${en?"Poland Research Hub":"מרכז מחקר פולין"}</span></a>
      <div style="display:flex;align-items:center;gap:.55rem;">
        <a class="geh-menu-button" href="${withPrefix(languagePath)}" aria-label="${en?"Switch to Hebrew":"Switch to English"}" style="text-decoration:none;"><i class="fa-solid fa-language"></i><span>${en?"עברית":"English"}</span></a>
        <button class="geh-menu-button" type="button" aria-expanded="false" aria-controls="gehDropdownMenu"><i class="fa-solid fa-bars"></i><span>${en?"Open navigation":"פתח ניווט"}</span><i class="fa-solid fa-chevron-down" aria-hidden="true"></i></button>
      </div>
    </div>
    <div class="geh-dropdown" id="gehDropdownMenu" hidden><div class="geh-menu-scroll"><div class="geh-menu-grid"></div></div></div>`;
    document.body.classList.add("geh-old-header-hidden");document.body.insertBefore(nav,document.body.firstChild);const grid=nav.querySelector(".geh-menu-grid");
    ...
    main.appendChild(link(withPrefix("jewish-timeline.html"),en?"Jewish Timeline":"ציר זמן יהודי",en?"Expanded Jewish timeline":"ציר זמן יהודי מורחב","fa-solid fa-timeline"));
    main.appendChild(link(withPrefix("folklore-archive.html"),en?"Folklore Archive":"ארכיון פולקלור",en?"Stories, proverbs and memory":"סיפורי עם, פתגמים וזיכרון","fa-solid fa-masks-theater"));
    ...
    const cities=section(en?"City research pages":"דפי מחקר ערים");(en?cityLinksEn:cityLinksHe).forEach(([path,label,note])=>cities.appendChild(link(cityPath(path),label,note,"fa-solid fa-city")));
    const language=section(en?"Language":"שפה");language.appendChild(link(withPrefix(languagePath),en?"עברית":"English",en?"Switch to Hebrew":"מעבר לאנגלית","fa-solid fa-language"));language.appendChild(link("#top",en?"Top of page":"ראש הדף",en?"Return to the beginning":"חזרה לתחילת העמוד","fa-solid fa-arrow-up"));
